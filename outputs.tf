...
output "vm_run_command_ids" {
  ...
  value       = { for name, rc in azurerm_virtual_machine_run_command.windows_vm : name => rc.id }
}

output "vm_run_command_instance_view" {
  description = "Instance view of azurerm_virtual_machine_run_command objects"
  value       = { for name, rc in azurerm_virtual_machine_run_command.windows_vm : name => rc.instance_view }
}

output "vm_run_command_locations" {
  description = "Azure region where each run-command resource is created"
  value       = { for name, rc in azurerm_virtual_machine_run_command.windows_vm : name => rc.location }
}

output "vm_run_command_names" {
  description = "Name property of each run-command resource"
  value       = { for name, rc in azurerm_virtual_machine_run_command.windows_vm : name => rc.name }
}

output "vm_run_command_script_uris" {
  description = "Script URIs for commands defined via script_uri"
  value = {
    for name, rc in azurerm_virtual_machine_run_command.windows_vm : name => try(rc.source[0].script_uri, null)
  }
}
...
output "vm_run_command_scripts" {
  ...
  value = {
    for name, rc in azurerm_virtual_machine_run_command.windows_vm : name => try(rc.source[0].script, null)
  }
}
